...
dates.forEach(tempdate => {
  ...
  * body is parsed into a JSON string and stored in a file (C:/ObsceneOddsAPIData/NBA/Games/<current-date>.json)
  ...
  */
  ...
  if(!fs.existsSync('/ObsceneOddsAPIData/NBA/Games/${tempdate}.json')){
    request(options, function (error, response, body) {
      ...
      fs.writeFile(`/ObsceneOddsAPIData/NBA/Games/${tempdate}.json`, data, function (err) {
        if (err) throw err;
        counter = counter + 1;
      })
    });
